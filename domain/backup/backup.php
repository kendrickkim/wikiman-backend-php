...
function wiki_backup_download()
{
    if (wiki_db_driver() !== "sqlite") {
        wiki_abort(501, "BACKUP_DRIVER_UNSUPPORTED", ["driver" => wiki_db_driver()]);
    }
    ...
    try {
        ...
        @unlink($path);
        wiki_abort($error->status, "BACKUP_DOWNLOAD_FAILED");
    } catch ( Throwable $error ) {
        @unlink($path);
        error_log("wikiman: backup download failed - " . $error);
        wiki_abort(500, "BACKUP_DOWNLOAD_FAILED");
    }
}
...
function wiki_backup_inspect_route()
{
    ...
    try {
        ...
        wiki_abort($error->status, "BACKUP_DATABASE_CORRUPT");
    } catch ( Throwable $error ) {
        error_log("wikiman: backup inspect failed - " . $error);
        wiki_abort(400, "BACKUP_DATABASE_CORRUPT");
    }
}
...
function wiki_backup_restore_route()
{
    ...
    try {
        ...
        wiki_abort($error->status, "BACKUP_RESTORE_FAILED");
    } catch ( Throwable $error ) {
        error_log("wikiman: backup restore failed - " . $error);
        wiki_abort(400, "BACKUP_RESTORE_FAILED");
    }
}
